...
public class OnSpawnProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof Level _level)
			_level.getScoreboard().addPlayerTeam("stand_user_invisible");
		if (world instanceof Level _level)
			_level.getScoreboard().addPlayerTeam("stand_user_visible");
		if (world instanceof Level _level)
			_level.getScoreboard().addPlayerTeam("non_stand_user");
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
					("team modify " + "stand_user_visible" + " seeFriendlyInvisibles true"));
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
					("team modify " + "stand_user_invisible" + " seeFriendlyInvisibles false"));
		if (world instanceof ServerLevel _level)
			_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(x, y, z), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
					("team modify " + "non_stand_user" + " seeFriendlyInvisibles false"));
		{
			boolean _setval = false;
			entity.getCapability(JojowosModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
