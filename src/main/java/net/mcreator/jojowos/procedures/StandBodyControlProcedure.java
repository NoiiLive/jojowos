...
public class StandBodyControlProcedure {
	public static void execute(Entity entity) {
		...
		double cooldown = 0;
		usemove = false;
		subtract = ((double) MoveDirectoryConfiguration.BODYCONTROLCDMIN.get() - (double) MoveDirectoryConfiguration.BODYCONTROLCDMAX.get())
		...
		if (subtract != 0) {
			cooldown = (double) MoveDirectoryConfiguration.BODYCONTROLCDMAX.get() + Math.round((entity.getCapability(JojowosModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JojowosModVariables.PlayerVariables())).StandSpeed * subtract);
		} else {
			cooldown = (double) MoveDirectoryConfiguration.BODYCONTROLCDMAX.get();
		}
		nocooldown = 100;
		move = "Body Control";
		...
		if (usemove == true) {
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal("Move WIP"), true);
		}
	}
}
