...
public class StandKickBarrageProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		String move = "";
		usemove = false;
		subtract = ((double) MoveDirectoryConfiguration.KICKBARRAGECDMIN.get() - (double) MoveDirectoryConfiguration.KICKBARRAGECDMAX.get())
		...
		if (subtract != 0) {
			cooldown = (double) MoveDirectoryConfiguration.KICKBARRAGECDMAX.get() + Math.round((entity.getCapability(JojowosModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JojowosModVariables.PlayerVariables())).StandSpeed * subtract);
		} else {
			cooldown = (double) MoveDirectoryConfiguration.KICKBARRAGECDMAX.get();
		}
		nocooldown = 60;
		move = "Kick Barrage";
				...
				if (((entity.getCapability(JojowosModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new JojowosModVariables.PlayerVariables())).StandSkin).equals("Shadow")) {
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("jojowos:stw_kick_barrage")), SoundSource.PLAYERS, 1, 1);
						} else {
							_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("jojowos:stw_kick_barrage")), SoundSource.PLAYERS, 1, 1, false);
						}
					}
					...
					if (entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("jojowos:stand_mob")))) {
						if ((entityiterator instanceof TamableAnimal _tamEnt ? (Entity) _tamEnt.getOwner() : null) == entity) {
							...
							if (entityiterator instanceof TheWorldShadowEntity) {
								((TheWorldShadowEntity) entityiterator).setAnimation("kickbarrage");
							}
